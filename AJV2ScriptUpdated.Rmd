...
#### R Version 1.4.1103 © 2009-2021 RStudio, PBC ; As of 8 April 2021 No new updates ####
...
# here in this result the average you calculate is the same for each sample across all genotypes. basically, you get only 3 different averages. Are you sure you are grouping by the right variables? My suspect is that you want to group by genotype, spores and sample and then calculate the mean. I have left commented code to do that below. 
avg_po1 <- all_po_data %>% 
  ...
  select(Genotype, Spores, Sample, mean_po) %>% 
...
# here instead of set names, we can use a left_join 
...
animal_mean <- all.female %>%
  ...
  summarise(mean_surface_area = mean(Size_mm2, na.rm = TRUE),
            ...
            ss_fr = mean_feeding_rate/mean_final_surface_area,
...
filtered_df2 <-  animal_mean %>% 
  dplyr::group_by(Genotype, Spore_level) %>%
  mutate(n_obs = n()) %>% 
  summarise(pinf = (inf/n_obs), na.rm = TRUE) %>% 
  distinct() %>% as.data.frame()
...
# Need this color scheme to work  
  Pfeed <- ggplot(filtered_df, aes(x = Spore_level, color = Spore_level, y = ss_fr))+
...
# Longevity/survival  
# Need this color scheme to work
  Psurv <- ggplot(filtered_df, aes(x = Spore_level, color = Spore_level,  y = survival))+
    stat_summary(fun.data  = mean_se, size = 1, alpha = 1.75,
                 position = position_dodge(0.2))+
    geom_point(position = position_dodge(0.2), size = 4, alpha = 0.40)+
    #scale_color_manual(values = c("#563caa", "#aa3c90", "#90aa3c", "#3caa56", "continuous"), name = "Spore level") +
    singlefigtheme+
    facet_wrap(~Genotype, nrow = 1)+
  ...
  # Growth
  # Need this color scheme to work
    Pgrow <- ggplot(filtered_df, aes(x = Spore_level, color = Spore_level,  y = mean_growth_length))+
    stat_summary(fun.data  = mean_se, size = 1, alpha = 1.75,
                 position = position_dodge(0.2))+
    geom_point(position = position_dodge(0.2), size = 4, alpha = 0.40)+
    #scale_color_manual(values = c("#563caa", "#aa3c90", "#90aa3c", "#3caa56", "continuous"), name = "Spore level") +
    ...
    # Need this color scheme to work
    Pbirth <- ggplot(filtered_df, aes(x = Spore_level, color = Spore_level,  y = (fecundity/mean_final_surface_area)))+
    stat_summary(fun.data  = mean_se, size = 1, alpha = 1.75,
                 position = position_dodge(0.2))+
    geom_point(position = position_dodge(0.2), size = 4, alpha = 0.40)+
    #scale_color_manual(values = c("#563caa", "#aa3c90", "#90aa3c", "#3caa56", "continuous"), name = "Spore level") +
    ...
    # Need this color scheme to work
    Pspores <- ggplot(filtered_df, aes(x = Spore_level, color = Spore_level,  y = spores_ul/mean_final_surface_area))+
    stat_summary(fun.data  = mean_se, size = 1, alpha = 1.75,
                 position = position_dodge(0.2))+
    geom_point(position = position_dodge(0.2), size = 4, alpha = 0.45)+
    #scale_color_manual(values = c("#563caa", "#aa3c90", "#90aa3c", "#3caa56", "continuous"), name = "Spore level") +
    singlefigtheme+
    facet_wrap(~Genotype, nrow = 1)+
  ...
  RTPlot1 <- plot_grid(Pinf, Pfeed, Psurv, Pgrow, Pbirth, Pspores, nrow = 6, align = "v")
  RTPlot1
...
#create matrix that will store the values
deltaB_values <- matrix(nrow = length(unique(animal_mean$Genotype)), ncol = 4)

#index outside the loop
index <- 1

for (id in unique(animal_mean$Genotype)) {
  #use & to create a subset based on more than one conditions
  #& means and, | means or in R logic
  data0 <- subset(animal_mean, Spore_level == 0 & Genotype == id)
  b0  <- (data0$fecundity)
  data75 <- subset(animal_mean, Spore_level == 150 & Genotype == id)
  b150  <- (data75$fecundity)
  data300 <- subset(animal_mean, Spore_level == 300 & Genotype == id)
  b300    <- (data200$fecundity)
  deltab150   <-  ((b150 -b0)/b0)*100
  deltab300  <-  ((b300 - b0)/b0)*100
  #add data to empty matrix
  deltaB_values[index, 1] <- id
  #if value is empty use NA else input the value
  deltaB_values[index, 2] <- ifelse(is_empty(deltab150), NA, deltab150)
  deltaB_values[index, 3] <- ifelse(is_empty(deltab300), NA, deltab300)
  
  index <- index + 1
}

deltaB_values <- as.data.frame(deltaB_values)
deltaB_values
names(deltaB_values) <-c("Genotype","150","300")
deltaB_values
```


```{r net reproductive rate}
# This section needs to calculate the net reproductive rate for a Genotype/spore combination is obtained by multiplying the proportion of females surviving to each age (lx) by the average number of offspring produced at each age (mx) and then adding the products from all the age groups: R0 = Σlxmx. 
...
# 4)  add the products from all age groups: R0 = Σlxmx. (again, per genotype and spore level)
...
#here we get an error, because you are trying to subset the exp_av dataframe based on the inf column, but the inf column is not in the exp_av dataframe! we forgot to add the inf in the dplyr::select above. 
