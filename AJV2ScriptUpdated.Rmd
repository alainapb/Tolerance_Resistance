...
```{r PlotTheme}
  singlefigtheme <- theme_classic() + theme(axis.title.x = element_text(size = 18, face = "bold"),
...
animal_mean <- all.female %>%
  ...
  summarise(mean_surface_area = mean(Size_mm2, na.rm = TRUE),
            ...
            survival = mean(Time till death_days, na.rm = TRUE),
...
# Proportion infected
 Pinf <- ggplot(filtered_df2, aes(x = Spore_level, color = as.factor(Spore_level),  y = pinf))+
  stat_summary(fun.data = mean_se, size = 2,
               alpha = 0.5, position = position_dodge(0.2))+
    geom_point(position = position_dodge(0.2), size = 4, alpha = 0.7)+
    singlefigtheme+
...
# Feeding rate - size specific (see Appendix for analyses illustrating links between body size and feeding rates)
  Pfeed <- ggplot(filtered_df, aes(x = Spore_level, color = Spore_level, y = ss_fr))+
    stat_summary(fun.data = mean_se, size = 2, alpha = 0.5, position = position_dodge(0.2))+
    geom_point(position = position_dodge(0.2), size = 4, alpha = 0.7)+
    #scale_color_manual(values = c("#563caa", "#aa3c90", "#90aa3c", "#3caa56", "continuous"), name = "Spore level") +
    singlefigtheme+
...
# Longevity/survival  
  Psurv <- ggplot(filtered_df, aes(x = Spore_level, color = Spore_level, shape = Genotype, y = age_at_death))+
    stat_summary(fun.data  = mean_se, size = 2, alpha = 0.5,
                 position = position_dodge(0.2))+
    geom_point(position = position_dodge(0.2), size = 4, alpha = 0.7)+
    scale_color_manual(values = wes_palette("GrandBudapest2", 6, type = "continuous"))+
    singlefigtheme+
    facet_wrap(~Genotype, nrow = 1)+
  ...
  # Growth
    Pgrow <- ggplot(filtered_df, aes(x = Spore_level, color = Spore_level, shape = Genotype, y = log(TotMass)))+
    stat_summary(fun.data  = mean_se, size = 2, alpha = 0.5,
                 position = position_dodge(0.2))+
    geom_point(position = position_dodge(0.2), size = 4, alpha = 0.7)+
    scale_color_manual(values = wes_palette("GrandBudapest2", 6, type = "continuous"))+
    ...
    Pbirth <- ggplot(filtered_df, aes(x = Spore_level, color = Spore_level, shape = Genotype, y = (fecundity/mean_final_surface_area)))+
    stat_summary(fun.data  = mean_se, size = 2, alpha = 0.5,
                 position = position_dodge(0.2))+
    geom_point(position = position_dodge(0.2), size = 4, alpha = 0.7)+
    scale_color_manual(values = wes_palette("GrandBudapest2", 6, type = "continuous"))+
    ...
    Pspores <- ggplot(filtered_df, aes(x = Spore_level, color = Spore_level, shape = Genotype, y = spores_ul/mean_final_surface_area))+
    stat_summary(fun.data  = mean_se, size = 2, alpha = 0.5,
                 position = position_dodge(0.2))+
    geom_point(position = position_dodge(0.2), size = 4, alpha = 0.7)+
    scale_color_manual(values = wes_palette("GrandBudapest2", 6, type = "continuous"))+
