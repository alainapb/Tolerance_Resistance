...
# spore.names <- as.character(unique(animal_mean$Spore_level))
# spore.names
# 
# filtered_df2 <-  animal_mean %>% 
#   dplyr::group_by(Genotype, Spore_level, Animal) %>%
#   mutate(n_susc = sum(suc, na.rm = TRUE),
#     
#   )
#   summarise(mean_pinf = mean(inf/(inf + susc)), na.rm = TRUE)  %>% 
#   select(Genotype, Spore_level,  pinf) %>% 
#   distinct() %>% as.data.frame()
# filtered_df2
...
## I need the rest of the data to be plotted here...the geom_point command. 
## Under Genotype 3  
Pinf <- ggplot(animal_mean, aes(x = Spore_level,  y = 100*inf/(inf + susc) , color = as.factor(Spore_level)))+
  ...
  #geom_rect(xmin = 4.5, xmax = Inf, ymin = -Inf,ymax = Inf, fill = "grey", color = "NA", alpha  = 0.30)+
...
Pfeed <- ggplot(animal_mean, aes(x = Spore_level, color = as.factor(Spore_level), y = ss_fr))+
  geom_rect(xmin = -Inf, xmax = 15.5, ymin = -Inf,ymax = Inf, fill = "grey", color = "NA", alpha = 0.01)+
