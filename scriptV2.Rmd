...
exp_av <- filtered_df %>%
  ...
  summarise(fr = mean(mean_feeding_rate, na.rm = T), #average feeding rate
   ...
   dplyr::select(c(1,2,3,4,5,9, 10, 11,15,16,18)) # in general, it's not great coding practice, for reproducability to use these type of numbers - you also can't easily error check! so make sure to always use the names 
...
infection <- animal_mean %>%
  ...
  dplyr::select(c(1,2,3,6)) # in general, it's not great coding practice, for reproducability to use these type of numbers - you also can't easily error check! so make sure to always use the names 
...
animal_mean %>%
  filter(inf == 1) %>%
  filter(mean_sc_fr < .8) %>%
  ggplot(aes(y=((mean_final_length^3)-(mean_init_length^3)), x=mean_sc_fr, color = Genotype), size = 5)+
    geom_point(aes(shape=as.factor(Spore_level)))+
    geom_smooth(method="lm", alpha = 0.2)+
    scale_color_brewer(palette = "Set2")+
    labs(x= "Size corrected feeding rate", y = "Growth")+
    theme_bw()+
    theme(panel.border = element_rect(color="black", fill="NA", size=1),
          axis.title.x=element_text(color="black", size = 17),
          axis.title.y=element_text(color="black", size = 17),
