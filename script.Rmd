...
#control only Midland 281 do NOT have a correlation either
with(filter(animal_mean, Genotype == "A'45", Spore_level == 0), plot(fr_sw ~ mean_size_animal))
with(filter(animal_mean, Genotype == "A'45", Spore_level == 0), cor.test(fr_sw, mean_size_animal))
...
po %>% 
  ggplot()+
  geom_point(aes(x=Spores, y = cor_delta_po, size = Prop_inf))+
  facet_grid(~Genotype)
...
##Does level of exposure and infection status predict the amount of active PO?
po %>%
  ggplot(aes(x=Spores, y=Prop_inf, size=cor_delta_po, color=cor_delta_po))+
...
po %>%
  ggplot(aes(x=Spores, y=cor_delta_po, color=Genotype, size=Prop_inf)) + 
  geom_point() +
  theme_classic()
...
#with interaction : Spore levle significant
fit1<-aov(cor_delta_po ~ Genotype + Spores * Prop_inf, data=po) 
summary(fit1)
#without interaction : Spore level and Genotype significant
fit2<-aov(cor_delta_po ~ Genotype + Spores + Prop_inf -1 , data=po) 
summary(fit2)
...
summary(lm(cor_delta_po ~ Genotype + Spores + Prop_inf -1, data=po))
...
(ggplot(po, aes(x=Spores, y=cor_delta_po))+
    geom_point()+
    geom_smooth(method="lm")+
    theme_classic())
...
(ggplot(po, aes(x=Prop_inf, y=cor_delta_po))+
    geom_point()+
    geom_smooth(method="lm")+
    theme_classic())
...
ggplot()+
    geom_point(po, mapping=aes(x=Spores, y=cor_delta_po, fill=Prop_inf),
               size=5, pch=21)+
    facet_grid(.~Genotype)+
    theme_classic()+
    scale_x_continuous(breaks = c(0,150,300), limits = c(-20, 320))+
...
PO<-ggplot()+
    geom_point(po_av, mapping=aes(x=Spores, y=av_delta_po),
               size=5, pch=22)+
