...
```{r begin set up for MLE, include = TRUE}
## 'data' is a dataframe with columns:
## 'Age' host age at time of exposure
## 'P0' parasite/pathogen spore dose
## 'Animal' biological replicate number
## 'ID' a numeric identifier for each day-by-age treatment (added below)
...
ID <- as.numeric(data$Plate_Treatment)
ID
...
sobolDesign(lower = c(u1 = log(0.0001),         ## age/time-specific per-spore susceptibility
                      u2 = log(0.0001),
                      u3 = log(0.0001),
                      u4 = log(0.0001),
                      f1 = log(0.001),          ## age/time feeding rate
                      f2 = log(0.001),
                      f3 = log(0.001),
                      f4 = log(0.001),
...
nll2 <- function(params, data, mod) {
  ...
  for (id in unique(data$ID)) {                                  ## For each age.day value, compute the NLL of its set of parameter values and add this to the existing total
    
    fhat <- c(exp(params["f1"]),                                 ## transform age/day-specific feeding rate back to the natural scale and choose the fhat value for this genotype
              exp(params["f2"]),
              exp(params["f3"]),
              exp(params["f4"]),
              exp(params["f5"]))[id] %>% unname
...
```{r calculate the likelihood of all the parameter sets using nll2, include = TRUE} 
...
fits <-foreach(i = 1:nrow(pars2)) %dopar%{
  ...
  subplex(par = unlist(pars2[i,]), 
          ...
          mod ='mod4',
          hessian = FALSE,
          control = list(maxit = 25000) # set to 10 just for testing code
  ) -> fits[[i]]
}
...
```{r check for convergence, include = TRUE} 
...
```{r PLOT PROBABILITY OF INFECTION ACROSS SIZES OR SPORE LEVELS, include = TRUE} 
