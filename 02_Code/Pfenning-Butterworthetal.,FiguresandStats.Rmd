...
ggsave("FeedingRates.png", Fig3, height = 10, width = 10, dpi = 600)
...
plotgrowth <- inf3 %>% 
  ...
  scale_x_continuous(limits = c(0.05, 0.36), breaks = c(0.10, 0.20, 0.30))
plotgrowth
```
...
plotsporeyield <- inf3 %>% 
  ...
  ggplot(aes(y = meanspores, 
             ...
             shape = as.factor(Spore_level),
             color = as.factor(Genotype)))+
  geom_smooth(aes(y = meanspores, x = meanfeedrate, group =1), method="lm", color = "black", alpha = 0)+
  geom_point(position = position_dodge(0.2), size = 5)+
