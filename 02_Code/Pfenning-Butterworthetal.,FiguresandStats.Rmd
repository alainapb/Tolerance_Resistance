...
genolabel <- c("Genotype 4 ", "Genotype 3", "Genotype 2 ", "Genotype 1") 
names(genolabel) <- c("A45", "Midland 252", "Midland 281", "Standard")
```

#### Alternative options considered for plot 2

#### Multi-panel with susceptibility first and then spore yield
...
previnf1 <- 
  ...
  geom_jitter(size = 5, width = 10, height = 0.5, alpha = 0.3)+
...
# combine figures
Figure2<-previnf4 + previnf3 + previnf2 + previnf1 
Figure2
#ggsave("Figure2_v2.png", Figure2, height = 10, width = 10.5, dpi = 600)
...
spores3 <- 
  ...
  scale_color_manual(values = c("darkcyan", "goldenrod1", "darkmagenta" , "continuous"), name = "Spore level") +
  singlefigtheme+ 
  labs(x= expression("Spore dose (no. mL"^-1*")"), y = expression("Susceptibility (infection prevalence)"))+
  facet_grid(~ Genotype,
             labeller = labeller(Genotype = genolabel)) +
  theme(panel.spacing = unit(1.05, "lines"),
        panel.border = element_rect(color = "black", fill = NA, size = 0.25),  
...
spores1 <- 
  ...
  scale_color_manual(values = c("darkcyan", "goldenrod1", "darkmagenta" , "continuous"), name = "Spore level") +
  singlefigtheme+
  labs(x = expression(""),y = expression("Susceptibility (infection prevalence)"))+
  facet_grid(~ Genotype,
             labeller = labeller(Genotype = genolabel)) +
    theme(panel.spacing = unit(1.05, "lines"),
        panel.border = element_rect(color = "black", fill = NA, size = 0.25),  
...
feed4 <- 
  ...
  filter(Genotype=="A45") %>%
  filter(Infected_Uninfected != "Exposed") %>%
  ggplot(aes(x = Spore_level, fill = as.factor(Spore_level), color = as.factor(Spore_level), y = meanfeedrate)) +
  geom_rect(xmin = -Inf, xmax = 15.5, ymin = -Inf, ymax = Inf, fill = "lightgrey", color = "NA", alpha = 0.01) +
  stat_summary(fun.data = mean_se, size = 1.5, alpha = 1.75,  position = position_dodge(0.2)) +
  ...
  theme(panel.spacing = unit(1.05, "lines"),
        ...
        legend.text = element_text(size = 8),
        strip.text.x = element_text(size = 12))+
  scale_y_continuous(limits = c(0.05, 0.5), breaks = c(0.0, 0.1, 0.2, 0.3,0.4))+
  scale_x_continuous(limits = c(0, 300), breaks = c(0, 150, 300))
...
feed3 <- 
  ...
  scale_fill_manual(values = c("darkcyan", "goldenrod1", "darkmagenta"))+
  scale_color_manual(values = c("darkcyan", "goldenrod1", "darkmagenta" , "continuous"), name = "Spore level") +
  labs(x = expression("Pathogen dose (no. mL"^-1*")"), y = "") +
  singlefigtheme+
  facet_grid(~ Genotype,
             labeller = labeller(Genotype = genolabel)) +
  theme(panel.spacing = unit(1.05, "lines"),
...
# combine figures
Figure3 <- feed1 + feed3 + feed4 
Figure3
Fig3<-Figure3 + theme(legend.position = 'right')


ggsave("FeedingRates.png", Fig3, height = 7, width = 12, dpi = 600)
...
ggplot(data = filter(inf3, Genotype != "Midland 281"), aes(y = probinf*100, 
             x = meanfeedrate))+
...
genolabel <- c("Genotype 1",  "Genotype 3", "Genotype 4") 
names(genolabel) <- c("Standard", "Midland 252","A45")
...
Figure_appendix <- (plota + plotb) /(plotc + plotd)
Figure_appendix
...
otherwise, infection had little effect on lifespan (at least over the
limited time frame captured in this assay - a great avenue for future
exploration)
...
Infection (and immune responses) and little effect on first day of
reproduction
...
# Table 4b_1 (accounting for genetic variation)
